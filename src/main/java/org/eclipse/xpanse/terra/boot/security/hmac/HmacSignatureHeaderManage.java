...
package org.eclipse.xpanse.terra.boot.security.hmac;
...
@ConditionalOnProperty(
        name = "terra.boot.webhook.hmac.request.signing.enabled",
        havingValue = "true",
        matchIfMissing = true)
public class HmacSignatureHeaderManage {
    ...
    public HmacSignatureHeaderManage(
            @Value("${terra.boot.webhook.hmac.request.signing.key}") String hmacSecretKey,
            @Value("${terra.boot.webhook.hmac.request.signing.algorithm}") String hmacAlgorithm)
